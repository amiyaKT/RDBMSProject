...
cursor.execute("SELECT * FROM ratings WHERE user_id = 18;")
...
for tup in ratings:
    ...
    corr_OneManOut_sorted = corr_OneManOut.sort_values('book', ascending=True)
    
    
    for i in range(0, len(book_correlation_values)):
        
        book_correlation_values[i][1] += corr_OneManOut_sorted.iloc[i][0]

book_correlation_values_df = pd.DataFrame(book_correlation_values)
book_correlation_values_sorted = book_correlation_values_df.sort_values(1, ascending=False)
recommended_books = []
for i in range(0, len(book_correlation_values)):
